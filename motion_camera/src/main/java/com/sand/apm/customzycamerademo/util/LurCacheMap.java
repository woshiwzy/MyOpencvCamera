...
public class LurCacheMap {
    ...
    public static Bitmap getBitmap(int targetWidth, int targetHeight,Bitmap.Config config) {
        String key = targetWidth + "x" + targetHeight+"_"+config;
        Bitmap bitmap = mLruCache.get(key);
        if (null == bitmap) {
            bitmap = Bitmap.createBitmap(targetWidth, targetHeight,config);
